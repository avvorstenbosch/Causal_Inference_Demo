...
```{r fig11, out.width="80%", fig.align = "center"}
ggplot() +
...
* Hmm, our model has very little predictive power, let's update the DAG

# Updated DAG

```{r fig12, out.width="80%", fig.align = "center"}
dag_r <- dagitty('dag {
bb="-4.361,-3.907,6.053,5.052"
EducationParents [pos="0.165,-2.506"]
ExtraEducationSupport [pos="-2.624,0.509"]
...
```{r fig13, out.width="80%", fig.align = "center"}
ggplot() +
...
```
```{r}
summary(model_math_r2)
```
 At this point it is clear the DAG is broken, the results make no sense. 
 Other than the fact that previous grades are a good indication of new grades
